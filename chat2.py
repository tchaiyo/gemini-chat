            ...
            if st.button('เริ่มใช้งาน'):
                ...
                cache = client.caches.create(
                    ...
                    config=types.CreateCachedContentConfig(
                        display_name='KMS', # used to identify the cache
                        system_instruction=(
                            'ใช้ข้อมูลต่อไปนี้เพื่อตอบคำถาม ตอบให้กระชับเข้าใจง่าย หากไม่รู้ให้ตอบว่าไม่รู้'
                        ),
                        contents=st.session_state.parts,
                        ttl="600s"
                    )
                )
